    ...
    private class ClassEmitter(
        ...
        fun emitClass(cls: IrClass, topLevel: Boolean) {
            ...
            val hasCompanion = !isObject &&
                (staticFields.isNotEmpty() || staticMethods.isNotEmpty() || clinit != null)
            ...
            if (hasCompanion) {
                if (wrote) code.newLine()
                emitCompanion(cls, staticFields, clinit, staticMethods)
                wrote = true
            }
        ...
        private fun emitProperty(cls: IrClass, field: IrField) {
            val isFinal = KotlinModifiers.has(field.accessFlags, KotlinModifiers.FINAL)
        ...
        private fun emitInitBlock(cls: IrClass, method: IrMethod) {
            emitErrorComment(method)
            code.attachDefinition(methodRef(cls, method))
            code.add("init {").newLine()
            code.incIndent()
            MethodBodyWriter(code, imports, method, NameGenerator(), emptyList()).writeBody()
            code.decIndent()
            code.attachNodeEnd()
            code.add("}").newLine()
        }
