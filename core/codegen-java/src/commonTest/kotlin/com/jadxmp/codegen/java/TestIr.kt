...
/**
 * Hand-construction helpers for `core:ir` values, so codegen tests build IR directly (no pipeline).
 */
object Flags {
...
fun generate(cls: IrClass): String = JavaCodeGenerator().generate(cls).code
...
/** A wrapped sub-expression operand. */
fun expr(insn: Instruction): InstructionOperand = InstructionOperand(insn)

/**
 * A source-level local variable that yields fresh, but linked, register operands via [ref] — every
 * [ref] shares one SSA value / LocalVar, so codegen gives them one name and reuses it.
 */
class Local(
    val regNum: Int,
    val type: IrType,
...
/** A bare (un-fused) `new <type>` allocation with no constructor call folded in. */
...
/** A `new <arrayType>{elements}`. [arrayType] is the WHOLE array type. */
fun filledNewArray(arrayType: IrType, elements: List<Operand>, result: RegisterOperand? = null): Instruction =
