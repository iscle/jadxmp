...
class JavaRegionTest {
    ...
    private fun useCall(vararg args: com.jadxmp.ir.insn.Operand): Instruction =
        staticInvoke(foo, "use", IrType.VOID, args.map { it.type }, args.toList())

    /** Wrap [region] in `void m(int a, int b) { … }` and generate the source. */
    ...
    private fun IrMethod.region0(r: Region) {
        this.region = r
    }
    ...
    fun forLoop() {
        ...
        assertThatCode(withRegion(loop))
            .containsOne("for (int i = 0; i < 10; i = i + 1) {")
    ...
    fun switchStatement() {
        ...
        assertThatCode(withRegion(r))
            ...
            .containsLine(2, "case 1:")
            .containsLine(2, "case 2:")
            .containsLine(2, "case 3:")
            .containsLine(2, "default:")
    ...
    fun multiCatch() {
        ...
        assertThatCode(withRegion(r)).containsOne("} catch (IOException | RuntimeException e) {")
    }
    ...
    fun duplicatedBlockRedeclaresBlockLocalTempPerCopy() {
        // A DUPLICATED straight-line block (the SAME block object placed in both arms of an `if`) that
        // carries a BLOCK-LOCAL temp `t` must re-declare `t` locally in EACH copy — the first copy's
        // declaration is out of scope in the sibling arm. The temp's defining instruction is marked
        // BLOCK_LOCAL_TEMP by the structuring stage; an UNMARKED value keeps its single declaration
        // (deduped), never re-declared per copy (a cross-block var is hoisted elsewhere).
        val t = Local(3, IrType.INT, name = "t") // block-local temp: defined + used within the shared block
        val u = Local(4, IrType.INT, name = "u") // unmarked value: deduped, declared once
        ...
        assertThatCode(withRegion(r))
            .countString(2, "int t = a + b;") // block-local temp re-declared in EACH copy
            .countString(1, "int u = a + 1;") // unmarked value declared ONCE (deduped, not per-copy)
    }
}
